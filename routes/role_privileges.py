...
Routes for managing schema-level and object-level privileges for roles.
"""
from fasthtml.common import Div, P, Tr, Td
...
def register_routes(app):
    """Register all role privilege management routes."""

    @app.rt('/role/get-schema-tables/{schema_name}')
    def get(session, schema_name: str):
        """Get tables for a schema as select options."""
        try:
            rs = get_rs(session)
            tables = rs.get_schema_tables(schema_name)
            # Return as HTML options
            html_options = ''.join([f'<option value="{t}">{t}</option>' for t in tables])
            return Div(html_options)
        except Exception as e:
            add_toast(session, f'Error loading tables: {str(e)}', 'error', True)
            return Div()

    @app.rt('/role/get-schema-views/{schema_name}')
    def get(session, schema_name: str):
        """Get views for a schema as select options."""
        try:
            rs = get_rs(session)
            views = rs.get_schema_views(schema_name)
            # Return as HTML options
            html_options = ''.join([f'<option value="{v}">{v}</option>' for v in views])
            return Div(html_options)
        except Exception as e:
            add_toast(session, f'Error loading views: {str(e)}', 'error', True)
            return Div()

    @app.rt('/role/get-schema-functions/{schema_name}')
    def get(session, schema_name: str, frm_data: dict):
        """Get functions or procedures for a schema as select options."""
        try:
            rs = get_rs(session)
            func_type = frm_data.get('new-func-type-' + schema_name, 'FUNCTION')
            ...
            # Return as HTML options
            html_options = ''.join([f'<option value="{f}">{f}</option>' for f in funcs])
            return Div(html_options)
        except Exception as e:
            add_toast(session, f'Error loading functions: {str(e)}', 'error', True)
            return Div()

    @app.rt('/role/load-table/{schema_name}')
    def post(session, schema_name: str, frm_data: dict):
        """Load table privilege checkboxes form row."""
        try:
    ...
    @app.rt('/role/load-view/{schema_name}')
    def post(session, schema_name: str, frm_data: dict):
        """Load view privilege checkboxes form row."""
        try:
    ...
    @app.rt('/role/load-function/{schema_name}')
    def post(session, schema_name: str, frm_data: dict):
        """Load function/procedure privilege checkboxes form row."""
        try:
    ...
    @app.rt('/role/save-privileges')
    def post(session, frm_data: dict):
        """Save role privilege changes."""
        try:
    ...
    @app.rt('/role/schema-nav/{schema_name}')
    def get(session, schema_name: str):
        """Get schema navigation for role."""
        try:
            schemas = session.get('schemas', get_rs(session).get_all_schemas())
            # Return navigation component (placeholder)
            return Div(P(f"Schema navigation for {schema_name}"))
        except Exception as e:
            return Div(P(f"Error loading schema navigation: {str(e)}"))

    @app.rt('/role/schema-content/{role_name}/{schema_name}')
    def get(session, role_name: str, schema_name: str):
        """Get schema content for role."""
        try:
            rs = get_rs(session)
